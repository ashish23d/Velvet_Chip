...
const ProductForm: React.FC<ProductFormProps> = ({ productToEdit, categories, onSave, onCancel }) => {
  ...
  // State for dynamic attributes (JSONB)
  ...
  useEffect(() => {
    if (productToEdit) {
      ...
      // Initialize attributes state safely
      let initialAttributes = productToEdit.attributes || {};
      ...
      setAttributes(initialAttributes && typeof initialAttributes === 'object' ? initialAttributes : {});
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const productData = {
      ...
      tags: tags,
      attributes: attributes, // Add attributes to product data
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      {/* Dynamic Attributes */}
      <div className="p-6 bg-white dark:bg-gray-800 rounded-lg shadow">
        <h3 className="text-lg font-medium leading-6 text-gray-900 dark:text-white mb-2">Product Attributes</h3>
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
          Add custom attributes like Brand, Material, Fabric, etc. These will appear as dynamic filters on the category page.
        </p>
        <div className="space-y-3">
          {!attributes || Object.keys(attributes).length === 0 ? (
            <p className="text-sm text-gray-500 italic">No attributes defined.</p>
          ) : (
            Object.entries(attributes).map(([key, value]) => (
              <div key={key} className="flex items-center gap-2">
                <input
                  ...
                  value={key}
                  readOnly
                  className={`${inputClass} flex-1 bg-gray-50 dark:bg-gray-900 cursor-not-allowed`}
                ...
                <button
                  ...
                  className="p-2 text-gray-400 hover:text-red-500 rounded-full hover:bg-red-50 dark:hover:bg-red-900/10 transition-colors"
          ...
          <button
            type="button"
            onClick={() => {
              const key = prompt("Enter attribute Name (e.g., Brand, Material, Fabric):");
              if (key) {
                const trimmedKey = key.trim();
                // Prevent duplicate keys/empty
                ...
                setAttributes(prev => ({
                  ...prev,
                  [trimmedKey]: '' // Initialize empty value
                }));
              }
            }}
            className="flex items-center gap-1 text-sm font-medium text-primary hover:text-pink-700 mt-2"
          >
          ...
          <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-100 dark:border-blue-800">
            <p className="text-xs text-blue-700 dark:text-blue-300">
              <strong>Tip:</strong> Consistent naming is key for filters. Use "Brand" instead of "company" or "brandName".
            </p>
          </div>
        </div>
      </div>
