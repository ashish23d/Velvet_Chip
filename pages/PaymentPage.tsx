...
const PaymentPage: React.FC = () => {
  const { cart, placeOrder, checkoutState, currentUser } = useAppContext();
  ...
  }, [currentUser, cart, checkoutState, navigate]);
  ...
  const handlePayment = async () => {
    ...
    if (activeTab === 'cod') {
        ...
        try {
            const newOrderId = await placeOrder('COD');
            if (newOrderId) {
    ...
    // For online payments, start the animation.
    // The order placement is handled by onAnimationComplete
            ...
            <div className="w-1/3 border-r border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-900/50 rounded-l-lg">
                {paymentTabs.map(tab => (
                    <button
                        ...
                        className={`w-full flex items-center gap-3 p-4 text-left text-sm font-medium transition-colors ${
                            activeTab === tab.id
