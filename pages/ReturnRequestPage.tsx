...
const ReturnRequestPage: React.FC = () => {
  ...
  const { currentUser, submitReturnRequest, getOrderById } = useAppContext();
  
  const order = useMemo(() => getOrderById(orderId), [orderId, getOrderById]);
  const itemToReturn = useMemo(() => order?.items.find(i => i.id === itemId), [order, itemId]);
  ...
  useEffect(() => {
    if (!currentUser) {
      navigate(`/login?redirect=${window.location.hash.substring(1)}`);
    }
        ...
        <div className="max-w-3xl mx-auto">
            <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md border border-gray-200 dark:border-gray-700">
              ...
              <div className="flex items-center gap-4 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg mb-6">
                <SupabaseImage bucket={BUCKETS.PRODUCTS} imagePath={itemToReturn.product.images[0]} alt={itemToReturn.product.name} className="w-20 h-28 object-cover rounded-md" />
                <div>
                    <h2 className="font-semibold text-gray-800 dark:text-white">{itemToReturn.product.name}</h2>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Order #{orderId}</p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">Size: {itemToReturn.selectedSize} | Color: {itemToReturn.selectedColor.name}</p>
                </div>
              </div>
