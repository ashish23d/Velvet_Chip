...
const OrderSummary: React.FC<OrderSummaryProps> = ({ cart, ctaText, ctaLink, onClick, disabled = false, isPaymentPage = false }) => {
  const { checkoutState, applyPromotion, removePromotion, getAvailablePromotions, taxSettings, categories, deliverySettings } = useAppContext();
  const { appliedPromotion, discount, deliveryType } = checkoutState;
  ...
  const deliveryCharge = (isPickup || subtotal >= freeThreshold) ? 0 : baseCharge;
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 sticky top-24">
      ...
      {/* Tax Breakdown */}
      {/* Tax Breakdown - Always show if enabled */}
      {taxSettings?.enabled && (
        <div className="border-t border-dashed border-gray-300 py-3 mt-2 space-y-1">
          <div className="flex justify-between text-gray-600 text-sm">
            <span>Base Price (Excl. Tax)</span>
            <span>₹{(subtotal - taxAmount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          </div>
