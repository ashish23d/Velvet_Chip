...
const AddressSelectionModal: React.FC<AddressSelectionModalProps> = ({ isOpen, onClose, onSelect }) => {
  ...
  return (
    ...
    >
      <div
        className="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-full max-w-2xl transform transition-all max-h-[90vh] flex flex-col"
      ...
      >
          ...
          {showForm ? (
            ...
            <div className="space-y-4">
              {addresses.map(addr => (
                ...
                >
                  <div className={`p-4 rounded-lg border-2 transition-all ${selectedAddressId === addr.id ? 'border-primary bg-primary/5 dark:bg-primary/20' : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:border-primary/50'}`}>
                    <h3 className="font-bold text-gray-800 dark:text-gray-100">{addr.name}</h3>
                    <div className="mt-2 space-y-1 text-sm text-gray-600 dark:text-gray-300">
                      <p>{addr.address}</p>
                      <p>{addr.city}, {addr.state} - {addr.pincode}</p>
                      <p className="pt-2">Mobile: <span className="font-medium text-gray-800 dark:text-gray-200">{addr.mobile}</span></p>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {!showForm && (
          <div className="p-4 flex-shrink-0 flex justify-end gap-3 border-t bg-gray-50 dark:bg-gray-800/50 dark:border-gray-700 rounded-b-lg">
